...
export async function GET(){
  ...
  const sql=db();
  const rows=await sql`SELECT u.id,u.email,u.role,u.active,u.must_change_password,u.last_login_at,u.created_at,u.updated_at,
...
export async function POST(req:Request){
  ...
  const row=(await sql`INSERT INTO app_users(email,password_hash,role,created_by,must_change_password) VALUES(${email},${hash},${role},${s!.email},${forcePasswordChange}) RETURNING id`)[0];
  ...
  await writeAudit({actor:s!.email,action:"CREATE_APP_USER",entityType:"app_user",entityId:String(row.id),next:{email,permissions,mustChangePassword:forcePasswordChange}});
...
export async function PATCH(req:Request){
  ...
  const sql=db(); const prev=(await sql`SELECT id,email,role,active,must_change_password FROM app_users WHERE id=${id}`)[0]; if(!prev)return NextResponse.json({error:"Usuario inexistente"},{status:404});
  ...
  await writeAudit({actor:s!.email,action:"UPDATE_APP_USER",entityType:"app_user",entityId:String(id),previous:{...prev,permissions:previousPermissions},next:{active:b.active,passwordReset:Boolean(b.password),forcePasswordChange:b.password?b.forcePasswordChange!==false:undefined,permissions:newPermissions}});
